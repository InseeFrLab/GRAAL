...
- **`MatchVerifier`** — vérifie qu'une correspondance libellé ↔ code proposée est valide ; sortie : booléen de validité, confiance, explication, et `p_match` (voir plus bas). C'est cet agent qui porte le cas d'usage « monitoring du modèle en production » (cf. cadrage §1.3).

Ces deux agents **ferment** une chaîne : ils rendent un jugement en un coup, là où les classifieurs explorent. D'où trois traits communs, qui les distinguent du reste des agents :

- **aucun outil** (`get_tools` → `[]`, `get_max_turns` → 1). Ils n'en appelaient aucun en pratique — les traces du *MatchVerifier* montrent 1,00 génération par ligne, zéro appel d'outil — mais les schémas des cinq outils du graphe leur coûtaient ~500 tokens de prompt par appel et laissaient ouverte la possibilité d'un détour multi-tours sur une question qui n'en demande pas. À la place, leur prompt contient directement la **notice officielle complète** des codes concernés (`Graph.get_notice`) : garantie présente, là où un appel d'outil dépendait du bon vouloir du modèle.
- **une règle de décision explicite**, pour le *MatchVerifier*. « Est-ce que ce code correspond ? » n'en est pas une : les libellés SIRENE sont tronqués, abrégés, souvent plus vagues que la nomenclature, et sans critère le modèle s'en invente un — strict, qui rejetait l'imprécision autant que l'erreur (`'avec chauffeur' -> 4933H` rejeté comme « trop vague »). Le prompt tranche : **on rejette une incompatibilité, pas une imprécision**. Un libellé qui décrit une partie plausible du champ du code est une correspondance valide, même s'il pourrait aussi relever d'un autre code.
...
- `confidence`, auto-déclarée par le modèle. Mal calibrée par construction : on observait 0,90 en moyenne sur `is_match=true` et 0,50 sur `false`, c'est-à-dire un modèle qui reporte P(le code est bon) plutôt que sa confiance dans son propre verdict. La description du champ le dit maintenant explicitement, mais un seuil sur cette colonne reste peu interprétable.
...
Le raisonnement du modèle (bloc `<think>` de Qwen3) est par ailleurs **désactivé** pour le *MatchVerifier* (`enable_thinking=False`, paramétrable au constructeur pour rejouer la comparaison) : le schéma de sortie demande déjà l'explication avant le verdict, donc le raisonnement-avant-conclusion est payé deux fois, et la seconde fois il est jeté au parsing.
...
Pour une campagne à plusieurs annotateurs, l'app est déployée plutôt que lancée en local : le `Dockerfile` à la racine construit l'image (dépendances installées depuis `uv.lock`) et `.github/workflows/docker.yml` la pousse sur Docker Hub — `inseefrlab/graal-match-verifier-review`, tags `latest`, `sha-<court>` et semver — à chaque push sur `main`. Les manifests Kubernetes, eux, ne vivent **pas** ici : ils sont dans le dépôt GitOps [`codif-ape-cd`](https://github.com/InseeFrLab/codif-ape-cd), sous `graal-match-verifier-review/` (avec un README listant les secrets à créer) et `argocd-templates/graal-match-verifier-review.yaml`, parce qu'un manifest dupliqué dans deux dépôts est un manifest qui diverge — c'est celui que lit Argo CD qui fait foi. Le run revu y est passé explicitement en `--commit`/`--model` : l'image n'embarque pas `.git`, donc la valeur par défaut — le tag de HEAD — n'a pas de sens en conteneur. Le déploiement tient à un seul réplica (`strategy: Recreate`) parce que l'ajout d'une ligne au JSONL est, sur S3, une réécriture complète de l'objet : deux processus concurrents perdraient des jugements. À l'intérieur d'un processus, Flask servant les requêtes sur des threads, l'app sérialise elle-même l'append et la réécriture du parquet derrière un verrou.
