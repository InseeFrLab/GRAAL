...
- `p_match`, calculée en Python : P(`is_match` = true) lue dans les **logprobs** du token `true`/`false` effectivement généré, renormalisée sur ces deux candidats (`p_match_from_logprobs`). Gratuite — les logprobs portent sur des tokens de toute façon décodés, aucun token supplémentaire — et c'est la probabilité du modèle, pas son opinion sur sa probabilité. Elle devait permettre de choisir *a posteriori* un point de fonctionnement (seuil) au lieu de subir celui que le prompt impose, et donc de tracer une courbe ROC plutôt que de mesurer un point unique.

> **Le raisonnement a largement neutralisé cette propriété**, et c'est le coût caché de `enable_thinking=True`. Sur le run de 2 000 lignes, seules **114 lignes sur 1 999 (5,7 %)** ont un `p_match` entre 0,02 et 0,98 : le modèle, ayant déjà tranché dans son bloc `<think>`, émet le token booléen quasiment sans hésitation. Le taux de rejet est donc **plat d'un seuil à l'autre** (20,91 % à 0,1 comme à 0,75, 20,96 % à 0,9) — il n'y a plus de courbe à parcourir, seulement le point que le prompt impose. Arbitrer précision/rappel *a posteriori* suppose maintenant de seuiller `match_score`/`alternative_score`, qui sont eux étalés, plutôt que `p_match`.
...
### 5.1 Évaluation du *MatchVerifier* (`match_verifier_eval.py` + `match_verifier_suggestions.py` + `apps/match_verifier_eval_app.py`)

Le *MatchVerifier* (§3.5) porte le cas d'usage « monitoring » : il faut donc savoir ce que vaut son verdict, et pas seulement s'en servir. La chaîne tient en trois scripts, dont le deuxième est facultatif.
...
- **`match_verifier_suggestions.py`** *(facultatif)* — précalcule, pour chaque ligne d'un run, les cinq codes les plus plausibles que l'app proposera à l'annotateur. Répondre « quel serait le bon code ? » dans un champ libre suppose de connaître 1 059 positions de tête ; cliquer dans une liste courte ne le suppose pas. Trois sources, choisies pour ne pas partager leurs erreurs : le modèle supervisé de production (`SupervisedClassifier.top_k`, via codif-ape-API — la seule dont le score soit une probabilité calibrée, et le seul candidat sans composant commun avec le *MatchVerifier*), la recherche vectorielle sur les notices (`Graph.get_closest_codes`, celle dont part l'*Agentic RAG*), et le meilleur concurrent que le vérificateur a lui-même nommé sur la ligne (`match_verifier_alternative_code`, déjà dans le parquet). N'ayant pas d'échelle commune — une probabilité, une distance cosinus, un pourcentage sorti d'un LLM —, elles sont fusionnées **par leurs rangs** (*Reciprocal Rank Fusion* pondérée, cf. `fuse_candidates`) : un code que deux sources citent en 2ᵉ position passe devant un code qu'une seule place en 1ʳᵉ. Pondérée, parce qu'à poids égaux c'est le volume qui déciderait — la recherche vectorielle rend toujours ses cinq candidats là où le modèle supervisé s'arrête souvent au premier (il tronque de lui-même dès que la queue de distribution devient négligeable, ce qui est une information sur l'ambiguïté du libellé). Les codes sont normalisés avant fusion (`1071H` / `10.71H` selon la source), et le code déjà en place est retiré de la liste. Sortie : `match_verifier_suggestions.parquet` à côté du parquet du run, en format long (une ligne par `(row_id, rang)`, colonnes `code`, `name`, `sources`, `fusion_score`, `supervised_proba`), donc relisible en SQL pour mesurer après coup ce que le suggesteur valait. Une source qui échoue sur une ligne (l'API refuse un libellé vide, par exemple) est journalisée et laissée de côté, les autres continuent ; le script refuse en revanche d'écraser un fichier existant sans `--overwrite`, changer les suggestions au milieu d'une campagne changeant ce que les annotateurs ont vu sans que rien dans le log ne le dise. Le calcul se fait par `row_id` et non par ligne : le parquet d'un run répète des paires (libellé, code), que l'app n'affiche qu'une fois.
- **`apps/match_verifier_eval_app.py`** — app Flask de revue humaine de ce parquet. Pour chaque ligne : le libellé, le code avec sa notice officielle (Neo4j), et le verdict du *MatchVerifier* — accompagné de sa note de correspondance, du meilleur code concurrent trouvé et de la note de celui-ci, pour que l'annotateur voie contre quoi le verdict a été rendu. Les runs antérieurs au passage aux notes en pourcentage restent lisibles : leur `match_verifier_confidence` est relue en repli et les colonnes de score restent vides. L'annotateur répond à une question — le verdict est-il correct ? — plus le code qu'il aurait mis, choisi parmi les candidats précalculés quand le run en a (cf. `match_verifier_suggestions.py`) ou saisi à la main sinon. Une version antérieure posait en plus, séparément, « le code est-il correct ? », ce qui permettait de recalculer exactitude/précision/rappel de `is_match` sans faire confiance ni au label d'entraînement ni au verdict ; la question a été retirée du formulaire (coût d'annotation jugé supérieur à son apport), mais la colonne `human_code_correct` reste au log et au schéma, et `/metrics` continue d'afficher ces métriques sur les jugements déjà collectés — gelées, avec leur propre dénominateur. Même découpage par annotateur que `multi_method_review_app.py` (pool partagé pour l'accord inter-annotateurs + tranche propre à chacun, déterministe à partir de `--input`/`--reviewers`/`--shared-n`/`--unique-n`/`--seed`, donc revue asynchrone). Un annotateur qui n'était pas prévu au lancement peut s'ajouter lui-même depuis la page d'accueil : il donne son nom, le nombre de libellés qu'il veut relire et la part de sa tranche qui doit recouper celle des autres (défaut `--overlap-pct`, 30 %) — ce recouvrement est tiré en priorité parmi les lignes **déjà jugées** (recouper une ligne que personne n'a encore jugée ne donne de l'accord inter-annotateurs que si son propriétaire y vient), le reste parmi les lignes du parquet que personne n'a. Cette tranche-là dépend de l'état du moment — qui avait déjà quoi — et ne se recalculerait pas à l'identique : elle est écrite dans `<output-dir>/<commit>/<modèle>/match_verifier_assignments.json` puis relue au démarrage, et ses recouvrements rejoignent le pool partagé au calcul de l'accord. `--reviewers` peut donc être laissé vide. Quand le run a un `match_verifier_suggestions.parquet` à côté du sien, la question du code devient un sélecteur : les cinq candidats avec leur intitulé, leur notice dépliable et la mention des méthodes qui les ont proposés, plus « Autre code » et son champ libre, plus « je ne propose pas de code ». Le fichier restant facultatif, un run sans suggestions s'annote comme avant. Montrer une liste déplace les réponses vers elle, et ces candidats viennent de modèles qui partagent des composants avec celui qu'on évalue : la liste n'apparaît donc que sous la question du code, jamais sous « le verdict est-il correct ? » — la seule dont `/metrics` tire une métrique vivante —, et chaque jugement enregistre ce qui était affiché (`suggested_codes_shown`) et si le code retenu en vient (`human_suggested_code_source`), de quoi mesurer ce déplacement après coup plutôt que d'en débattre. `/metrics` en tire « codes pris dans les suggestions », qui se lit dans les deux sens : ce que vaut le suggesteur, et à quel point il oriente. Les jugements sont journalisés en JSONL append-only (idempotent par `(reviewer, row_id)`) et rematérialisés après chaque validation en parquet à côté, avec le nom de l'annotateur en colonne `reviewer`. L'app lit `<input>/<commit>/<modèle>/match_verifier_eval.parquet` et écrit dans `<output-dir>/<commit>/<modèle>/` : `--commit` et `--model` valent par défaut le tag de HEAD et `GENERATION_MODEL`, et si aucun run n'existe pour ce couple l'app refuse de démarrer en listant les runs disponibles (cf. §5.1.1). Le run en cours de revue (`<commit>/<modèle>`) est affiché sur chaque page.

```bash
uv run -m src.evaluation.match_verifier_eval --n-samples 500 --concurrency 10
# facultatif, mais à faire avant d'ouvrir la revue : les candidats proposés à l'annotateur
uv run -m src.evaluation.match_verifier_suggestions --commit v0.0.1-1-g2441f54 --model qwen3-6-35b-moe
