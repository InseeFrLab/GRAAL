...
- **L'espace d'embedding utilisé par l'Agentic RAG comme *warm start* n'avait jamais été évalué formellement**, seulement via le script exploratoire ponctuel ci-dessus (`explorations.py`). → **Diagnostic committé le 15/07** : `evaluate_embeddings.py` (script autonome, hors `src/evaluation/`) calcule l'accuracy@1 / recall@5 et l'exactitude hiérarchique d'un k-NN cosinus libellé → notice, repris dans `presentation/embeddings.qmd` avec projections UMAP/PaCMAP/t-SNE/PCA, cohésion/confusion inter-groupes et comparaison multi-modèles. Une exploration ponctuelle antérieure à ce commit, non versionnée, avait rapporté un résultat préoccupant (accuracy@1 = recall@5 = 0 % sur un échantillon de 30 libellés, ex. « FOOTBALL FEMININ » classé 373ᵉ/747) qui a motivé ce chantier. **Résultat du diagnostic committé, sur l'échantillon complet (5 181 libellés)** : accuracy@1 = 48,1 %, recall@5 = 77,5 %, exactitude hiérarchique croissante jusqu'à 77,7 % en section — ce résultat ne corrobore pas le 0 % initial (y compris en ré-exécutant sur le même échantillon de 30, qui donne 46,7 %/66,7 %). L'espace d'embedding est donc utilisable comme *warm start*, avec une marge confortable en recall@5 mais un top-1 encore insuffisant pour se passer de la vérification/correction du *Navigator* qui suit (cf. `docs/framework.md` §5.1/§8). Cause exacte du 0 % initial non tranchée (script non versionné à l'époque, donc non diffable) ; sans incidence sur la conclusion actuelle puisqu'elle repose sur le diagnostic committé et reproductible.
...
- **Analyse des embeddings** : formaliser les explorations déjà amorcées (projections UMAP/PaCMAP/t-SNE/PCA, k-plus-proches-voisins dans `explorations.py`) en un diagnostic reproductible de la qualité de l'espace d'embedding utilisé par l'*Agentic RAG* — séparabilité des codes, zones de confusion, sensibilité au modèle d'embedding choisi. → **Committé le 15/07** (cf. §2.2) : `evaluate_embeddings.py` et `presentation/embeddings.qmd`, accuracy@1 = 48,1 % / recall@5 = 77,5 % sur l'échantillon complet avec le modèle actuellement configuré. Reste à faire : comparer d'autres modèles d'embedding candidats (le diagnostic le permet déjà via `CANDIDATE_MODELS`, non encore exploité) pour voir si ce top-1 peut être amélioré.
...
| **S2** | 14 → 18 juillet | Formaliser la méthode d'évaluation (§3.1) et constituer le **jeu d'évaluation stratifié** (§3.2), versionné dans le dépôt — ✅ fait le 6/07 (`data/eval/eval_set.parquet`) — en s'appuyant sur un premier **diagnostic de l'espace d'embedding** — ✅ committé le 15/07 (`evaluate_embeddings.py`, `presentation/embeddings.qmd`, cf. §2.2), accuracy@1 = 48,1 % / recall@5 = 77,5 % sur l'échantillon complet. Brancher le classifieur *Agentic RAG* dans le pipeline principal — ✅ fait le 6/07. Nettoyage du dépôt (drift.py, scripts morts) — ✅ fait le 15/07. **Durcissement de la boucle *Navigator*** contre les blocages et les réponses non terminales silencieuses, et mise en cohérence de `run_eval.py` — ✅ fait le 15/07 (cf. §2.2). |
