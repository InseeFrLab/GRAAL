...
| **S2** | 14 → 18 juillet | Formaliser la méthode d'évaluation (§3.1) et constituer le **jeu d'évaluation stratifié** (§3.2), versionné dans le dépôt — ✅ fait le 6/07 (`data/eval/eval_set.parquet`) — en s'appuyant sur un premier **diagnostic de l'espace d'embedding** (formalisation des analyses UMAP/kNN déjà amorcées, reste à faire). Brancher le classifieur *Agentic RAG* dans le pipeline principal — ✅ fait le 6/07. |
