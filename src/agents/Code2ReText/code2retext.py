...
@hydra.main(
    version_base=None,
    config_path="../NaiveCode2Text/runtime_config",
    config_name="config"
    )
def main(cfg: DictConfig):
    ...
    for i in range(0, len(valid_items), cfg["llm"]["generation_batch_size"]):
        ...
        try:
            ...
            for item, generation in zip(batch, generations):
                ai_confidence = requests.post(DISCRIM_URL, json={"texts": generation.labels}).json()
    ...
    if cfg["export"]["output_format"] == ".txt":
        ...
        names = [r["name"] for r in results_buffer]
        labels = [r["labels"] for r in results_buffer]
        ai_confidence = [r["ai_confidence"] for r in results_buffer]
